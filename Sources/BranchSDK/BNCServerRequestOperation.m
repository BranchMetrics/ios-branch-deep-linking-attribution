...
- (void)start {
    ...
    // TODO: If tracking is disabled then we should prematurely fail and finish the operation.
    ...
    if (!Branch.trackingDisabled) {
        if (!([self.request isKindOfClass:[BranchInstallRequest class]])) {
            ...
            if (!preferenceHelper.randomizedDeviceToken || !preferenceHelper.sessionID || !preferenceHelper.randomizedBundleToken) {
                [[BranchLogger shared] logError:[NSString stringWithFormat:@"Missing session items (device token or session ID or bunlde token). Dropping request: %@", self.request.requestUUID] error:nil];
                BNCPerformBlockOnMainThreadSync(^{
                    [self.request processResponse:nil error:[NSError branchErrorWithCode:BNCInitError]];
                 });
                self.executing = NO;
...
- (void)cancel {
    ...
    if (!self.isExecuting) {
        ...
        // TODO: Discuss and Add cancellation mechanism for the underlying NSURLSessionTask if that makes sense.
