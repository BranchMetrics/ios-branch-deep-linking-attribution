...
- (void)testDefaultURIScheme_TestBed {
  //ND  XCTAssert([[BNCSystemObserver defaultURIScheme] isEqualToString:@"branchtest"]);
}

- (void)testAppVersion_TestBed {
    XCTAssert([[BNCSystemObserver applicationVersion] isEqualToString:@"1.0"]);
}

- (void)testBundleIdentifier_TestBed {
    NSString *bundleId = [BNCSystemObserver bundleIdentifier];
    XCTAssert([bundleId isEqualToString:@"branch.BranchSDKTestsHostApp"]);
}
