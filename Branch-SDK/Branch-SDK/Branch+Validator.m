...
- (void) startValidation {
    ...
    [[[BNCServerInterface alloc] init] getRequest:nil url:[preferenceHelper getAPIURL:endpoint] key:nil callback:^(BNCServerResponse *response, NSError *error) {
        if (error) {
            ...
            if ([teamID isEqualToString:errorString] || [bundleIdentifier isEqualToString:errorString] || [uriScheme isEqualToString:errorString] ) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"Branch integration verification" message:nil preferredStyle:UIAlertControllerStyleAlert];
                alertController.message = @"Please check your xCode logs for details";
                // Ok
                [alertController addAction:[UIAlertAction actionWithTitle:@"Got it" style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
                }]];
                [[UIViewController bnc_currentViewController] presentViewController:alertController animated:YES completion:nil];
            });
        }
    }];
}
